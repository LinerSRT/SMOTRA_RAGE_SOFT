...
public class LinerButton extends JButton {
    ...
    private final Font font = new Font("Yandex Sans Text Medium", Font.PLAIN, 12);
    ...
    public Dimension getMinimumSize() {
        ...
        return new Dimension(icon.getWidth() + textWidth + insets.left + insets.right, textHeight + 4);
    }
        ...
        switch (iconPosition) {
            case LEFT:
                graphics2D.drawImage(icon,
                        4, height / 2 - icon.getHeight() / 2, null
                );
            ...
            case RIGHT:
                graphics2D.drawImage(icon,
                        width - icon.getWidth() - 4, height / 2 - icon.getHeight() / 2, null
                );
                graphics2D.setColor(foregroundColor);
                Other.drawCenteredString(
                        graphics2D,
                        getText(),
                        new Rectangle(4, 0, width - icon.getWidth() - 4, height),
                        font
                );
                break;
            case CENTER:
                graphics2D.drawImage(icon,
                        4, 4, icon.getWidth() - 4, icon.getHeight() - 4, null
                );
                break;
        }
        if (isRippleEnabled) {
    ...
    public void setIcon(BufferedImage icon) {
        this.icon = ImageUtils.resize(icon, 16, 16);
        List<Color> accentColors = ImageUtils.getAccentColors(icon, Color.BLACK);
        setRippleColor(accentColors.get(0));
        setFocusColor(accentColors.get(0));
        repaint();
    }
